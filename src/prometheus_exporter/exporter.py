...
class Exporter(ClassLogger):
    ...
    def __init__(self, config_file="config.toml", name=None, labels=Labels(), no_config_file=False, *args, **kwargs) -> None:
    ...
    async def startup_tasks(self, *args, **kwargs) -> None:
        pass
    ...
    async def on_shutdown(self, app: Application) -> None:
        ...
        for task in all_tasks():
            if coro := task.get_coro():
                ...
                if task_name in ["_run_app", "asyncTearDown"]:
    ...
    def export_config_metrics(self, log_bump: int=10) -> None:
    ...
    async def handle_metrics(self, request: Request, *args, **kwargs) -> Response:
        ...
        self.logger.info(
            "[%s (%s)] Sending response: <%d> Length: %s"
            % (request.remote, request.query_string, response.status, response.content_length)
        )
        return response

    async def export(self, label_filter: dict[str, str] | None=None) -> str:
